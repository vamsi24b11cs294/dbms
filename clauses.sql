...
SELECT COUNT(gender) FROM employee WHERE gender='Male' AND department='SW';
SELECT name,age FROM employee WHERE salary>75000 ORDER BY age, name;

SELECT SUM(salary)FROM employee WHERE department='HR' AND age>=27;

SELECT name FROM employee WHERE (MAX(age));

SELECT name,date_of_joining FROM employee ORDER BY date_of_joining; 
SELECT MAX(salary),MIN(salary),MAX(salary)-MIN(salary) FROM employee WHERE gender='Female' AND department='HR';

SELECT AVG(salary) FROM employee WHERE department='Testing' AND date_of_joining>TO_DATE('02-09-2019','DD-MM-YYYY');

SELECT name FROM employee WHERE department='Testing' AND salary<45000;
SELECT * FROM EMPLOYEE;

SELECT department, COUNT(*) AS emp_count FROM employee GROUP BY department;
SELECT gender ,COUNT(gender) AS emp_count FROM employee GROUP BY gender;

SELECT department,MAX(salary),MIN(salary),SUM(salary) FROM employee GROUP BY department;

SELECT gender,MAX(age),MIN(date_of_joining),SUM(salary) FROM employee GROUP BY gender;

SELECT department,COUNT(*) AS emp_cnt 
FROM employee
WHERE salary>60000 
GROUP BY department ORDER BY emp_cnt;
