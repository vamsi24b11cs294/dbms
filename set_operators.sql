...
DBMS_OUTPUT.PUT_LINE('I AM SRUTHI');
DBMS_OUTPUT.PUT_LINE('FROM AUS ,CSE');
END;
/
DECLARE 
...
C NUMBER;
BEGIN 
--ASSIGNMENT OPERATOR ':= ' NOT = IT IS FOR COMPAROISON
A:=10;
B:=20;
C:=A+B;
...
B:=A*A;
C:=4*A;
DBMS_OUTPUT.PUT_LINE('AREA AND PERIMETER ARE :'|| B|| ' ' || C);
END;
/
DECLARE 
SAL1 NUMBER;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('JOIN_DATE: ' || TO_CHAR(join_date, 'DD-MON-YYYY HH24:MI:SS'));
END;
/
...
END;
/
-- First enable output (run this separately, not inside BEGIN..END)
SET SERVEROUTPUT ON;

-- Now run your PL/SQL block
DECLARE 
    doj DATE;
    today_date DATE;
    days NUMBER;
...
IF A>B THEN 
SMALL:=A;
DBMS_OUTPUT.PUT_LINE('SMALL BETWEEN A AND B IS :' || SMALL);
ELSE
SMALL:=B;
DBMS_OUTPUT.PUT_LINE('SMALL BETWEEN A AND B IS :' || SMALL);
END IF;
...
C:=&VAL3;
IF A>B AND A>C THEN
DBMS_OUTPUT.PUT_LINE('A IS GREATER OF A,B,C');
ELSIF B>C AND B>A THEN
DBMS_OUTPUT.PUT_LINE('B IS GREATER OF A,B,C');
ELSE 
DBMS_OUTPUT.PUT_LINE('C IS GREATER OF A,B,C');
END IF;
END;
/
SET SERVEROUTPUT ON;
...
/
BEGIN
    DBMS_OUTPUT.PUT('I AM SRUTHI');
    DBMS_OUTPUT.PUT('ADITYA UNIVERSERTY');
    DBMS_OUTPUT.PUT('CSE BRANCH');
    DBMS_OUTPUT.NEW_LINE();
END;
/
/
...
CREATE OR REPLACE 
FUNCTION add_two(a NUMBER , b NUMBER) RETURN 
NUMBER IS 
    c NUMBER;
BEGIN 
    c:=a*a+b*b;
...
SELECT wish('Sruthi','Female') FROM dual;
...
CREATE OR REPLACE FUNCTION is_divisible(a NUMBER, b NUMBER)
RETURN NUMBER IS
    s NUMBER;
BEGIN
    IF MOD(b, a) = 0 THEN
...
SELECT is_divisible(5,10) FROM dual;

CREATE OR REPLACE FUNCTION maximum(a NUMBER,b NUMBER,c NUMBER)
RETURN NUMBER IS
    n NUMBER;
BEGIN
    IF a>=b AND a>=c THEN
...
SELECT maximum(1,2,3) FROM dual;
...
SELECT MONTHS_BETWEEN(CURRENT_DATE,DATE '2024-09-11') AS months FROM dual;
